...
```{r problem 4.2}
df_hs_train$sqft_basement <- ifelse(df_hs_train$sqft_basement == 0, 0, 1)
df_hs_test$sqft_basement <- ifelse(df_hs_test$sqft_basement == 0, 0, 1)

str(df_hs_train)
```

We fit a regression model again to predict price using the train dataset and the r function lm:

```{r problem 4.3}
fit_price_train <- lm(price ~. , data = df_hs_train)
summary(fit_price_train)
coeff_ols <- fit_price_train$coefficients
```
```{r problem 4.4}
p <- length(fit_price_train$coefficients)
n <- length(df_hs_train$price)
ols_plot_resid_lev(fit_price_train)
ols_plot_resid_stud_fit(fit_price_train)
ols_plot_cooksd_chart(fit_price_train)
df_hs_train %>%
  dplyr::mutate(Observation = row.names(df_hs_train)) %>%
  dplyr::select(Observation, price) %>%
  cbind(dplyr::select(data.frame(stats::influence.measures(fit_price_train)$infmat), cook.d))  %>%
...
df_hs_train <- df_hs_train[!(row.names(df_hs_train) %in% case_lev),]
fit_price_train <- lm(price ~. , data = df_hs_train)
summary(fit_price_train)
par(mfrow=c(1,1))
plot(fit_price_train)
par(mfrow=c(1,1))
boxplot(fit_price_train$residuals)
```
...
colNames <- names(df_hs_train[, -1])
for(col in colNames){
  print(ggplot(df_hs_train, aes_string(x = col, y = colnames(df_hs_train)[1])) +
    geom_point() +
...
ols_step_both_p(fit_price_train, pent=0.35, prem=0.05)
ols_step_backward_p(fit_price_train, prem = 0.05)
ols_step_forward_p(fit_price_train, penter = 0.05)

step_price_both <- ols_step_both_p(fit_price_train, pent=0.35, prem=0.05)
fit_price_train_both <- step_price_both$model
summary(fit_price_train_both)

step_price_back <- ols_step_backward_p(fit_price_train, prem = 0.05)
fit_price_train_back <- step_price_back$model
summary(fit_price_train_back)

step_price_fwd <- ols_step_forward_p(fit_price_train, penter = 0.05)
fit_price_train_fwd <- step_price_fwd$model
summary(fit_price_train_fwd)
...
```{r problem 6.4}
summary(fit_price_train_both)$adj.r.squared
summary(fit_price_train_back)$adj.r.squared
summary(fit_price_train_fwd)$adj.r.squared

rmse(df_hs_train$price,fit_price_train_both$fitted.values)
rmse(df_hs_train$price, fit_price_train_back$fitted.values)
rmse(df_hs_train$price, fit_price_train_fwd$fitted.values)
```
...
```{r problem 6.5}
df_hs_train_new <- subset(df_hs_train, select = -c(floors, sqft_basement, 
                                                       sqft_lot15, week_of_year, grade_11_13,
                                                       season_Spring, season_Summer, season_Fall))

df_hs_test_new <- subset(df_hs_test, select = -c(floors, sqft_basement, 
                                                       sqft_lot15, week_of_year, grade_11_13,
                                                       season_Spring, season_Summer, season_Fall))

fit_price_train_step <- lm(price ~., data = df_hs_train_new)
coeff_step <- fit_price_train_step$coefficients
summary(fit_price_train_step)
dispRegFunc(fit_price_train_step)
```
...
```{r problem 7.1}
par(mfrow=c(1,1))
plot(fit_price_train_step)
par(mfrow=c(1,1))
boxplot(fit_price_train_step$residuals)
```
...
```{r problem 7.2}
bptest(fit_price_train_step, studentize = FALSE)
```
...
```{r problem 7.3}
ei <- fit_price_train_step$residuals
ad.test(ei)
```
...
```{r problem 7.4}
p <- length(fit_price_train_step$coefficients)
n <- length(df_hs_train_new$price)
ols_plot_resid_lev(fit_price_train_step)
ols_plot_resid_stud_fit(fit_price_train_step)
ols_plot_cooksd_chart(fit_price_train_step)
df_hs_train_new %>%
  dplyr::mutate(Observation = row.names(df_hs_train_new)) %>%
  dplyr::select(Observation, price) %>%
  cbind(dplyr::select(data.frame(stats::influence.measures(fit_price_train_step)$infmat), cook.d))  %>%
  dplyr::arrange(desc(cook.d)) %>%
  dplyr::filter(cook.d >= 0.04)
```
...
```{r problem 7.8}
par(mfrow=c(1,1))
bc <- boxcox(fit_price_train_step, lambda=seq(-1,1,by=.1))
...
```{r problem 7.9}
fit_price_train_trans <- lm(log(price) ~. , data = df_hs_train_new)
coeff_trans <- fit_price_train_trans$coefficients
summary(fit_price_train_trans)
par(mfrow=c(1,1))
plot(fit_price_train_trans)
par(mfrow=c(1,1))
boxplot(fit_price_train_trans$residuals)
```
...
```{r problem 7.11}
bptest(fit_price_train_trans, studentize = FALSE)
```
...
```{r problem 7.12}
ei_trans <- fit_price_train_trans$residuals
ad.test(ei_trans)
```
...
```{r problem 8.1}
ei_trains <- fit_price_train$residuals
abs_ei_trans <- abs(ei_trains)
fit_price_res <- lm(abs_ei_trans ~., data = df_hs_train[, -1])
#summary(fit_price_res)
res_fitted_values <- fit_price_res$fitted.values
w1 <- 1/(res_fitted_values^2)

fit_price_train_trans_w1 <- lm(price ~. , 
                               weights = w1, data = df_hs_train)
...
while (iteration < 200){
  ...
  fit_price_res_2 <- lm(abs_ei_trans_2 ~., data = df_hs_train[, -1])
  #summary(fit_price_res_2)
  res_fitted_values_2 <- fit_price_res$fitted.values
  w2 <- 1/(res_fitted_values_2^2)
  
  fit_price_train_trans_w2 <- lm(price ~. , 
                               weights = w2, data = df_hs_train)
...
df_coeff_step <- data.frame(coeff = names(coeff_step), coeff_step = coeff_step)
df_coeff_trans <- data.frame(coeff = names(coeff_trans), coeff_trans = coeff_trans)
df_coeff_wls <- data.frame(coeff = names(coeff_wls), coeff_wls = coeff_wls)

df_wls <- df_coeff_ols %>% 
            left_join(df_coeff_step, by=c('coeff')) %>%
            left_join(df_coeff_trans, by=c('coeff')) %>%
...
```{r problem 8.4}
x <- data.matrix(dplyr::select(df_hs_train, -price))
y <- df_hs_train$price
```
...
olsrr::ols_plot_resid_lev(fit_price_train)
df_hs_train %>%
  dplyr::mutate(Observation = row.names(df_hs_train)) %>%
  dplyr::select(Observation, price) %>%
  cbind(dplyr::select(data.frame(stats::influence.measures(fit_price_train)$infmat), cook.d)) %>% 
  dplyr::arrange(desc(cook.d)) %>%
  dplyr::filter(cook.d >= 0.04)
olsrr::ols_plot_cooksd_chart(fit_price_train)
```
...
```{r problem 8.12}
set.seed(1023)
rr.huber <- rlm(price ~., df_hs_train)
summary(rr.huber)
...
y_hat.ols <- predict(fit_price_train, newdata = df_hs_train)

y_hat.step <- predict(fit_price_train_step, newdata = df_hs_train_new)

y_hat.trans <- predict(fit_price_train_trans, newdata = df_hs_train_new)
y_hat.trans <- exp(y_hat.trans)

y_hat.weight <- predict(fit_price_train_trans_w2, newdata = df_hs_train)

y_hat.hubber <- predict(rr.huber, newdata = df_hs_train)
...
sse.step <- sum((y - y_hat.step)^2)
sse.trans <- sum((y - y_hat.trans)^2)
...
sse_models <- c("sse.ols", "sse.step", "sse.trans", "sse.weight", "sse.hubber", "sse.ridge", "sse.lasso", "sse.enet")
sse_values <- c(sse.ols, sse.step, sse.trans, sse.weight, sse.hubber, sse.ridge, sse.lasso, sse.enet)
...
rsq.step <- 1 - sse.step / sst
rsq.trans <- 1 - sse.trans / sst
...
rsq_models <- c("rsq.ols", "rsq.step", "rsq.trans", "rsq.weight", "rsq.hubber", "rsq.ridge", "rsq.lasso", "rsq.enet")
rsq_values <- c(rsq.ols, rsq.step, rsq.trans, rsq.weight, rsq.hubber, rsq.ridge, rsq.lasso, rsq.enet)
...
x_test <- data.matrix(dplyr::select(df_hs_test, -price))
y_test = df_hs_test$price

y_hat.ols <- predict(fit_price_train, newdata = df_hs_test)

y_hat.step <- predict(fit_price_train_step, newdata = df_hs_test_new)

y_hat.trans <- predict(fit_price_train_trans, newdata = df_hs_test_new)
y_hat.trans <- exp(y_hat.trans)

y_hat.weight <- predict(fit_price_train_trans_w2, newdata = df_hs_test)

y_hat.hubber <- predict(rr.huber, newdata = df_hs_test)
...
sse.step <- sum((y_test - y_hat.step)^2)
sse.trans <- sum((y_test - y_hat.trans)^2)
